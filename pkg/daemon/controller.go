...
func (c *Controller) recordProviderNetworkErr(providerNetwork string, errMsg string) error {
	...
	klog.Infof("recordProviderNetworkErr pod %s", c.localPodName)
	if currentPod.Annotations == nil {
		currentPod.Annotations = make(map[string]string)
	}
	if errMsg == "" {
		delete(currentPod.Annotations, fmt.Sprintf(util.ProviderNetworkErrMessageTemplate, providerNetwork))
	} else {
		currentPod.Annotations[fmt.Sprintf(util.ProviderNetworkErrMessageTemplate, providerNetwork)] = errMsg
	}
