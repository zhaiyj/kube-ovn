...
func (c *ovnClient) AddLogicalRouterPolicy(lrName string, priority int, match, action string, nextHop string, externalIDs map[string]string) error {
	...
	for _, policy := range policyList {
		...
		if found || policy.Action != action || (policy.Action == ovnnb.LogicalRouterPolicyActionReroute && nextHop != *policy.Nexthop) {
	...
	if len(duplicate) == len(policyList) {
		policy := c.newLogicalRouterPolicy(priority, match, action, nextHop, externalIDs)
...
func (c *ovnClient) DeleteLogicalRouterPolicyByNexthop(lrName string, priority int, nexthop string) error {
	policyList, err := c.listLogicalRouterPoliciesByFilter(lrName, func(route *ovnnb.LogicalRouterPolicy) bool {
		...
		return (route.Nexthop != nil && *route.Nexthop == nexthop) || *route.Nexthop == nexthop
	})
	if err != nil {
		return err
	}
...
func (c *ovnClient) newLogicalRouterPolicy(priority int, match, action string, nextHop string, externalIDs map[string]string) *ovnnb.LogicalRouterPolicy {
